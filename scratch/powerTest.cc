...
NS_LOG_COMPONENT_DEFINE ("powerTest");
...
void
NodeStatistics::AdvancePosition (Ptr<Node> node, int stepsSize, double stepsTime)
{
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("PStepSize", "Distance between steps", stepsSize);
  ...
  wifi.SetRemoteStationManager (manager, "DefaultTxPowerLevel", UintegerValue (maxPower),"PowerStepSize", UintegerValue (pstepsSize), "RtsCtsThreshold", UintegerValue (rtsThreshold));
  wifiPhy.Set ("TxPowerStart", DoubleValue (minPower));
  wifiPhy.Set ("TxPowerEnd", DoubleValue (maxPower));
  wifiPhy.Set ("TxPowerLevels", UintegerValue (powerLevels));
  //wifiPhy.Set ("PowerStepSize", UintegerValue (stepsSize));
